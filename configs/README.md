...
- ByteTrack仅保留最近128条已删除轨迹用于诊断；闭环业务生命周期保留60秒供录像尾段和交接收尾，随后清理event/tracker映射，同时移除对应上传资格，避免按过车数长期增长
...
- 事件证据按 `event_output_dir/evidence/YYYYMMDD/<event_id>/manifest.json` 聚合，所有阶段同时追加到 `event_output_dir/event_index.csv`；Web可按event ID、车牌、时间、车型和异常原因查找
- 上传达到最大重试次数后进入对应SQLite队列的 `dead_letter` 表，保存payload、失败时间、次数和最后错误；同一event ID的后续type一起留存并按组重新入队，不再静默删除
