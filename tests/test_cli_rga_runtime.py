...
class CliRgaRuntimeTests(unittest.TestCase):
    ...
    def test_configure_rga_environment_ignores_config_enable(self):
        config = {"video": {"rga_enable": True}}

        with patch.dict(os.environ, {}, clear=True):
            cli._configure_rga_environment(config)
            self.assertEqual(os.environ.get("CLEANINGCAR_RGA_DISABLE"), "1")
            self.assertIsNone(os.environ.get("CLEANINGCAR_RGA_ENABLE"))
    ...
    def test_explicit_environment_disable_has_highest_priority(self):
        config = {"video": {"rga_enable": True}}

        with patch.dict(
            os.environ,
            {"CLEANINGCAR_RGA_ENABLE": "1", "CLEANINGCAR_RGA_DISABLE": "1"},
            clear=True,
        ):
            cli._configure_rga_environment(config)
            self.assertEqual(os.environ.get("CLEANINGCAR_RGA_DISABLE"), "1")
